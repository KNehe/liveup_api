...
def generate_receptionist_stats(request):
    ...
    referrals_by_user = Referral.objects.filter(
        created_by=request.user).count()
    referrals_today_by_user = Referral.objects.filter(
        created_by=request.user, created_at__date=today).count()

    patients = Patient.objects.all().count()
    patients_by_user = Patient.objects.filter(
        created_by=request.user).count()
...
def generate_clinician_stats(request):
    ...
    referrals = Referral.objects.all().count()
    referrals_to_user = Referral.objects.filter(
        doctor=request.user).count()
